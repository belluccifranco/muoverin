...
public class MegaCrypt {
	...
	public static long[] decrypt_key(long[] a, long[] key) {
		...
		for (int i = 0; i < a.length; i += 4) {
			long[] part = aes_cbc_decrypt_a32(Arrays.copyOfRange(a, i, i + 4),
					key);
			for (int j = i; j < i + 4; j++) {
				sum[j] = part[j - i];
			}
	...
	public static String base64_url_encode(String data) {
		...
		try {
			data = new String(base64_url_encode_byte(
					(data.getBytes("ISO-8859-1")), true), "ISO-8859-1");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
	...
	public static long[] base64_to_a32(String s)
	...
	public static String decrypt_attr(String attributes, long[] key) {
		try {
			return new String(aes_cbc_decrypt(
	...
	public final static byte[] base64_url_encode_byte(byte[] sArr,
			boolean lineSep) {
		// Check special case
